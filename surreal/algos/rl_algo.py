...
class RLAlgo(Algo, metaclass=ABCMeta):
    ...
    def event_tick(self, event):
        ...
        Args:
            event (RLAlgoEvent): The RLAlgoEvent object describing this event.
        """
    ...
    def event_episode_ends(self, event):  #
        """
...
class RLAlgoEvent(object):
    """
    Simple class to wrap properties that could be useful information for an RLAlgo when it receives some event from
    the Env (e.g. a tick).
    """

    def __init__(self, env, actor_slots, actor_time_steps, *,
                 current_actor_slot=None, r=None, t=None, s_=None):
        """
        Args:
            env (Env): The Env that triggered this event.
            actor_slots (List[int]): The positions in the batch of actors that use this Algo.
            actor_time_steps (int): The time steps (across all actors that share this Algo).
            current_actor_slot (int): The position in the actor-batch (if applicable).
            r (Optional[np.ndarray]): The batch of rewards received when reaching the next state (s').
            t (Optional[np.ndarray]): The terminal signals received when reaching the next state (s').

            s_ (Optional[any]): The batch of next states (s'). NOTE: If `t` is True for an s' in the batch, then that
                s' is the first state of a new (reset) episode and `r` is the last reward received in the old episode.
        """
        self.env = env
        self.actor_slots = actor_slots
        self.actor_time_steps = actor_time_steps
        self.current_actor_slot = current_actor_slot
        self.r = r
        self.t = t
        self.s_ = s_
