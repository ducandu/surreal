...
from surreal.components.policies.supervised_predictor import SupervisedPredictor
...
from surreal.components.loss_functions.neg_log_likelihood_loss import NegativeLogLikelihoodLoss
...
class IntrinsicCuriosityWorldOptionModel(SupervisedModel):
    ...
    def __init__(
        ...
        self.state_encoder = SupervisedPredictor(
            network_spec=encoder_network, output_space=Float(shape=(num_features,), add_batch_rank=True),
            scope="state-encoder"
        )
        ...
        predictor_network = NeuralNetwork(
            ...
            NeuralNetwork.make(world_option_model_network, scope="wom-nn"),
            ...
            NeuralNetwork.make(post_phi_concat_network, scope="post-phi-concat-nn"),
            # The ReShape component for flattening all actions in arbitrary action core.
        ...
        super().__init__(
            supervised_predictor_spec=dict(
                ...
                output_space=Dict({
                    ...
                    "predicted_phi_": Float(shape=(self.num_features,))
    ...
    def update(self, nn_inputs, labels=None, time_percentage=None):
        ...
        step_op = self.optimizer.act(self.variables(), loss, loss_per_item, time_percentage)
