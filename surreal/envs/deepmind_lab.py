...
from surreal.utils.util import force_list, default_dict


class DeepmindLabEnv(Env):
    """
    Deepmind Lab Env adapter for RLgraph:
    ...
    def __init__(self, level_id, observations="RGB_INTERLEAVED", actions=None, frameskip=4, config=None,
        ...
        Args:
            ...
            observations (Union[str,List[str]]): String specifier(s) for the observation(s) to be used with the
                given level. Will be converted into either a (single) PrimitiveSpace or a Tuple (of BoxSpaces).
                See deepmind's documentation for all available observations.
            actions (Optional[List[dict]]): The RLgraph action spec (currently, only Int (shape=()) RLgraph action
                core are supported) that will be translated from and to the deepmind Lab actions.
    ...
    def step(self, actions):
        # Do the actual step.
        reward = self.level.act(action=self.action_list[actions], num_steps=self.frameskip)
    ...
    def define_actions(actions_spec=None):
        """
        Translates and maps Rlgraph Int(shape=()) actions - provided by user - to the correct deepmind Lab
        representation for the calls to `step`.

        Args:
            actions_spec (List[dict]): The discrete action definitions to be supported by this Env.

        Returns:
            tuple:
            - A lookup list of deepmind actions, where the slot is the RLgraph Int value
            and the items are numpy arrays (with dtype=np.intc) that are understood by deepmind Lab.
            - The RLgraph action Space (Int(shape=(), n)), where n is the number of discrete actions.
        """
        ...
        return lookup_list, Int(len(actions_spec))
    ...
    def define_observations(self, observation_spec):
        ...
        for observation_name in observation_spec:
            ...
            if "float" in str(observation_item["dtype"]):
                space = Float(shape=observation_item["shape"], dtype=observation_item["dtype"])
            elif "int" in str(observation_item["dtype"]):
                space = Int(shape=observation_item["shape"], dtype=observation_item["dtype"])
            elif "str" in str(observation_item["dtype"]):
                space = Text(shape=observation_item["shape"])
            else:
