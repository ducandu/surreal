...
from surreal.envs import VectorEnv, Env


class SequentialVectorEnv(VectorEnv):
    """
    Sequential multi-environment class which iterates over a list of envs
    to step them.
    """
    def __init__(self, num_environments, env_spec, num_background_envs=1, async_reset=False):
        """
            num_background_envs (Optional([int]): Number of envs asynchronously
                reset in the background. Need to be calibrated depending on reset cost.
            async_reset (Optional[bool]): If true, resets envs asynchronously in another run_thread.
        """
        self.environments = []

        for _ in range(num_environments):
            if isinstance(env_spec, dict):
                env = Env.make(env_spec)
    ...
    def step(self, actions, **kwargs):
        ...
        for i in range(self.num_environments):
            state, reward, terminal, info = self.environments[i].act(actions[i])
            states.append(state)
...
class ThreadedResetter(Thread):
    """
    Keeps resetting envs in a queue,
    ...
    def __init__(self, env_spec, num_environments):
        ...
        # Create a set of envs ready to use.
        for _ in range(num_environments):
            env = Env.make(env_spec)
            state = env.reset()
    ...
    def run(self):
        # Keeps resetting envs as they come in.
        while True:
